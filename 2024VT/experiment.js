...
// Call the function to load Doom
loadDoom();
...
function loadDoom() {
    // Load the JS-DOS script from a CDN
    const script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/npm/@js-dos/core@7.4.7/dist/js-dos.js';
    script.onload = () => {
        // Create a container element where Doom will be rendered
        const container = document.createElement('div');
        container.id = 'dosbox';
        document.body.appendChild(container);

        // Fetch the DOSBox emulator from js-dos
        Dos(container).ready((fs, main) => {
            // Mount Doom game files using a URL to the ZIP file containing the game
            fs.extract('https://js-dos.com/cdn/dos/doom.jsdos').then(() => {
                // Execute the Doom game
                main(['-c', 'doom.exe']);
            });
        }).catch((error) => {
            console.error("Error loading JS-DOS or Doom game:", error);
        });
    };
