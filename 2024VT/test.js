...
function addDynamicCSS() {
    ...
    .mytooltip .mytooltiptext {
        ...
        position: absolute;
        z-index: 99;
        bottom: 120%; /* Adjusted from 20% to allow room above */
        left: 50%;
    ...
    /* Additional adjustment for preventing cut-off */
    .mytooltip .mytooltiptext.right-align {
        left: auto;
        right: 0;
        transform: translateX(0);
    }
    
    .mytooltip .mytooltiptext.left-align {
        left: 0;
        transform: translateX(0);
    }
...
function positionTooltip(tooltipTextElem) {
        const rect = tooltipTextElem.getBoundingClientRect();
        const viewportWidth = window.innerWidth;
        const viewportHeight = window.innerHeight;
    
        // Check if the tooltip is going out of the viewport
        if (rect.right > viewportWidth) {
            tooltipTextElem.classList.add('right-align');
        } else if (rect.left < 0) {
            tooltipTextElem.classList.add('left-align');
        }
    
        if (rect.bottom > viewportHeight) {
            tooltipTextElem.style.bottom = 'auto';
            tooltipTextElem.style.top = '-100%'; // Adjust as needed
        }
    }
...
function applyTooltipsToObject(obj) {
        ...
        // Apply dynamic positioning after the tooltips are rendered
        setTimeout(() => {
            document.querySelectorAll('.mytooltip .mytooltiptext').forEach(positionTooltip);
